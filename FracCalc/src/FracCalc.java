...
public class FracCalc {
	...
	public static String produceAnswer(String userProb){
		breakUp(userProb); //splits up fraction into 3 parts
		
		intVal(firstFrac); //changes the first fraction from string val to int val
		firstFracnum = num;
		firstFracdem = dem;
		intVal(secondFrac); //changes the second fraction from string val to int val
		secondFracnum = num;
		secondFracdem = dem;

		findOperator(operator); //finds type of operator for problem

		calculateAnswer(); //calculates the fraction into 'num' and 'dem' but does not reduce	

		return reducingFraction(num, dem); //takes in 'num' and 'dem' and uses euclids alg to reduce (also takes care of negatives in this method)
	}
	...
	private static String reducingFraction(int n, int d) {	
		...
		if(Math.abs(fn) > Math.abs(fd)){ //if the reduced fraction should be a mixed number  
			...
			if(num1 > 0){
				return (whole + "_" + num1 + "/" + Math.abs(fd)); //if fraction reads y_x/1 and x is greater then 0
			...
			return ("-" + num/n1 + "/" + Math.abs(dem/n1)); //if it should return a regular fraction 
		}else{
	...
	private static void intVal(String frac){
		...
		if(frac.contains("_") || frac.contains("/")){ //if 'frac' it is a mixed number or it is a regular fraction
			...
			if(frac.contains("_")){ //if 'frac' is a mixed number
				...
				}																					   //then multiply by -1 because numP1 is negative
